...
exoplanets allow us to do statistical studies of the newfound worlds by
...
measure precise values of the radius of a transiting planet \citep[see
e.g.][]{Torres2012}. The determination of the stellar radius is in turn
dependent on the quality of the derived stellar parameters such as the effective
temperature.

We continue the work of \citet{Santos13} by deriving parameters in a homogeneous
way using the method described in \citet{Sousa2011}. This in turn allow us to
study planet hosting stars in a homogeneous way and thus new correlations may be
found, or higher statistical certainty will be gained on already discovered
correlations for planet hosting stars. Analyzing high quality spectra (high
...
parameters fast using standard spectroscopic methods. This works well for
...
\section{Data}
\label{sec:data}
We obtain spectra related to this work by using the UVES \citep{UVES}, FEROS
\citep{FEROS}, and FIES \citep{FIES} spectrographs. Other spectra were found in
various archives. Additionally, we use spectra from HARPS \citep{HARPS} and
ESPaDOnS. Some characteristics of the spectrographs are presented in
...
spectrograph, and in cases there are multiple observations, we include all
...
\citep{Sneden1973}, and ARES \citep{Sousa2015a} for an all-in-one tool. MOOG is
a radiative transfer code under the assumption of local thermodynamic
equilibrium (LTE). And ARES is a tool to automatically measure equivalent widths
...
\ion{Fe}{I} and \ion{Fe}{II} line EWs, and iii) measure abundances for XXX
...
using a stellar atmosphere model with a given set of atmospheric parameters,
...
abundances (through the measured EWs) and the excitation potential (EP) and
reduced EW ($\log(EW/\lambda)$) we can constrain $T_\mathrm{eff}$ and
$\xi_\mathrm{micro}$. By obtaining ionization balance between \ion{Fe}{I} and
\ion{Fe}{II} (that is the average abundance of all \ion{Fe}{I} lines are equal
to the average abundance of all \ion{Fe}{II} lines) we constrain $\log g$. Last,
we change the input [\ion{Fe}/\ion{H}] to match that of the average output
...
that it is already written, and usually there are good documentation in
...
With the line list from \citet{Adibekyan...} with 15 different elements it is
...
\begin{table*}[htb!]
    ...
    \begin{tabular}{lllllll}
      ...
      % Add alternative name and SNR
        Star      & $T_\mathrm{eff}$ (K) &  $\log g$ (dex)     &  [Fe/H] (dex)        &  $\xi_\mathrm{micro}$ (km/s) & $\xi_\mathrm{micro}$ fixed? & Spectrograph & SNR   \\  %Program ID \\
      \hline
