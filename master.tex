...
of equivalent widths for a list of iron lines and making use of iron ionization
and excitation equilibrium principles. For the catalogue, and whenever possible,
we used parameters derived in previous works published by our team, using
well-defined methodologies for the derivation of stellar atmospheric
parameters.}
% Results
{The stellar parameters for the 50 stars are presented and compared with
previously determined literature values. For the catalogue, we compile values
...
catalogue is available online. The data can be used for statistical studies of
...
a web interface at \url{http://www.astro.up.pt/resources/fasma/}. This works
...
tool\footnote{\url{http://www.astro.up.pt/resources/fasma/}} for analyzing
...
iii) abundance derivation for 15 elements, all described below. The model
...
\begin{itemize}
    ...
    \item Microturbulence is connected with the saturation of the stronger iron
          ...
          the reduced EW, i.e. the slope of abundance vs. the reduced EW is zero.
\end{itemize}
...
\begin{enumerate}
    ...
    \item If the new set of parameters have already been used in a previous
          ...
          and a sigma equal to the absolute value of the indicator.
...
between the parameters, we lower the slopes found a little and arrived to the
very simple equations above.
...
The atomic data were calibrated with the Sun as reference and solar abundances
from \citet{Anders1989}. The EWs are measured automatically with the ARES driver
of FASMA. The element abundance of each line is derived using the atmospheric
parameters of the stars obtained from the previous step. The final element
abundance is calculated from the weighted mean of the abundances produced by all
lines detected for a given element as described in \citet{Adibekyan2015b}.
...
\url{http://www.astro.up.pt/resources/fasma/}, where more information are
available on each of the driver.
...
Table~\ref{tab:results} along with the atmospheric parameters of the stars.
The SNR is measured automatically by ARES, but we note that ARES smooth the
spectra before measuring the SNR, hence it is listed higher than the actual SNR.
...
region at low $T_\mathrm{eff}$. The fact that we are not able to derive
...
Finally, we also removed 19 stars from our sample since the parameters for these
stars did not converge during the minimization procedure. The $T_\mathrm{eff}$
for these stars is either too hot or too cold for the EW method to work.
...
the literature values we listed previously in SWEET-Cat. The old parameters are
listed in Table~\ref{tab:oldSC}. Among 2461 stars discovered to be a planet
hosts, 21\% of these stars have been analyzed in the homogeneous way as
...
\begin{figure*}[tpb]
    ...
    four plots we show the typical error on the parameters.}
    \label{fig:update}
\end{figure*}
...
Below we discuss the systems (seven stars, eight exoplanets) where the radius or
mass of the stars changes more then 25\% and how this influences the planetary
parameters. The changes in radius for a star is primarily due to changes in
$\log g$, which can be used as an indicator of the evolutionary stage of a star.
...
radii when the atmospheric parameters comes from different methods. As have
